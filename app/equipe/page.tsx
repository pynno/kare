...
export default function MedicoPage() {
  ...
  function gerarPDFReceita() {
    ...
    doc.setFontSize(11)
    doc.text("DADOS DO PACIENTE", 15, 90)
    doc.setFontSize(10)
    ...
    doc.save(`receita_${nomePaciente.replace(/ /g, "_")}_${agora.replace(/[/:, ]/g, "_")}.pdf`)
  ...
  function gerarPDFAtestado() {
    ...
    if (atestado.observacoes) {
      doc.text(`Observações: ${atestado.observacoes}`, 15, 165)
    }
    ...
    doc.save(`atestado_${nomePaciente.replace(/ /g, "_")}_${agora.replace(/[/:, ]/g, "_")}.pdf`)
    ...
    <main className="min-h-screen" style={{background: "#F0FAF6"}}>
      <div style={{background: "#085041", padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between"}}>
        <div>
          ...
          <p style={{fontSize: "12px", color: "#9FE1CB", marginTop: "2px"}}>
            {medico ? `Dr(a). ${medico.nome} — CRM ${medico.crm}` : "Painel do médico"}
          </p>
        </div>
            ...
            {mostrarReceita && (
              <div style={{background: "#F8FDFB", border: "0.5px solid #9FE1CB", borderRadius: "12px", padding: "16px", marginBottom: "16px"}}>
                ...
                {receitaAssinada ? (
                  ...
                  <button onClick={gerarPDFReceita} style={{...btnVerde, width: "100%"}}>
                    Gerar e baixar receita (PDF)
                  </button>
                )}
              </div>
            )}
            ...
            {mostrarAtestado && (
              <div style={{background: "#F8FDFB", border: "0.5px solid #9FE1CB", borderRadius: "12px", padding: "16px", marginBottom: "16px"}}>
                ...
                {atestadoAssinado ? (
                  ...
                  <button onClick={gerarPDFAtestado} style={{...btnVerde, width: "100%"}}>
                    Gerar e baixar atestado (PDF)
                  </button>
                )}
              </div>
            )}
